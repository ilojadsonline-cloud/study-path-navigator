...
- **Dashboard "Progresso por Disciplina"**: card lista TODAS as disciplinas respondidas (não usar `.slice(0,6)` — isso ocultava as disciplinas mais novas/menores como Redação Oficial e Língua Portuguesa, gerando queixa de "não computa"). Dados vêm de `respostas_usuario ⋈ questoes` (banco), ordenados por volume.
