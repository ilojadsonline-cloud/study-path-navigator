...
- Tabelas: `simulados_semanais` (meta), `simulado_semanal_questoes` (gabarito oculto — RLS admin-only; coluna `anulada boolean` p/ questões anuladas), `simulado_semanal_tentativas` (1 por user via UNIQUE(simulado_id,user_id)). RPC `get_simulado_semanal_ranking(p_simulado_id)` (security definer, só authenticated) ordena por pontuação desc, tempo asc; situação: classificado (≥60 e **posição≤100**) / aprovado_nao_classificado / reprovado.
- **Identificação OBRIGATÓRIA no ranking do simulado semanal**: todos que finalizam aparecem SEMPRE com o nome real (`profiles.nome`, fallback 'Participante'). NÃO respeita `show_in_ranking` — privacidade só vale para os rankings de estudo (semanal/mensal/geral em `get_ranking`/`get_my_ranking_position`, que são facultativos).
- Edge function `simulado-semanal` (verify_jwt=false, valida JWT no código): ações `status|start|save|submit|results|annul|unannul`. Score é calculado SEMPRE no servidor (service role) a partir dos gabaritos; **questão `anulada` conta como acerto para TODOS**. `annul`/`unannul` (admin-only via has_role) marca a questão e RECALCULA todas as tentativas finalizadas. Timer de 4h (`duracao_minutos`) não pausa; auto-finaliza ao expirar; 1 tentativa única.
- Admin sobe a prova via Markdown na aba "Simulado Semanal" (`AdminSimuladoSemanalTab`), com validação obrigatória da distribuição do edital (exatamente 50 na proporção). Pode ativar/desativar, ver ranking, excluir e **Editar** (`SimuladoSemanalEditor.tsx`): edita dados cadastrais (título/datas/duração/descrição) e cada questão (enunciado/alternativas/gabarito/comentário/disciplina), além de anular/reverter anulação questão a questão.
