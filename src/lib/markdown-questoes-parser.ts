import { normalizarDisciplina } from "./edital-distribuicao";
...
export function parseMarkdownQuestoes(markdown: string): ParseResult {
  ...
  blocks.forEach((block, i) => {
    ...
    const disciplina = normalizarDisciplina(data.disciplina || "");
    if (!disciplina) {
    ...
    const alternativasOk = alts.every((a) => a !== null && a!.trim().length > 0);
    if (!alternativasOk) {
      const faltando = alts.map((a, idx) => (a ? null : "ABCDE"[idx])).filter(Boolean).join(", ");
      ignoradas.push({ bloco, motivo: `Alternativa(s) ausente(s): ${faltando}`, preview });
      return;
    }

    const gabRaw = norm(data.gabarito || "");
    const gabLetter = gabRaw.match(/[a-e]/);
    if (!gabLetter) {
      ignoradas.push({ bloco, motivo: "Gabarito ausente ou inválido (use A, B, C, D ou E)", preview });
      return;
    }
    ...
    validas.push({
      ...
      enunciado,
      alternativas: [alts[0]!.trim(), alts[1]!.trim(), alts[2]!.trim(), alts[3]!.trim(), alts[4]!.trim()],
      gabarito,
      comentario,
    });
  });
