-- 1) Coluna para anulação de questões do simulado semanal
ALTER TABLE public.simulado_semanal_questoes
  ADD COLUMN IF NOT EXISTS anulada boolean NOT NULL DEFAULT false;

-- 2) Retificação Edital nº 003/2026: ampliação para 100 vagas
--    Classificado = aprovado (>=60) dentro do limite de 100 vagas
...
AS $function$
  ...
  SELECT user_id, nome, pontuacao, acertos, total, finished_at, duracao_segundos, posicao,
    CASE
      WHEN pontuacao >= 60 AND posicao <= 100 THEN 'classificado'
